...
class Game {
    constructor(nRow, nCol) {
        ...
        // Obstacles
        const obstacles = [ [0, 1], [2, 0], [4, 5], [2, 3], [2, 2], [2, 4] ];
        obstacles.forEach(point => this.board[point[0]][point[1]].visited = true);
    }
    
    getPlayerCellIndices(player) {
        let cells = [];
        this.board.forEach((row, j) => row.forEach((cell, i) => {
    ...
    toString() {
        ...
        this.board.forEach(row => {
            ...
            string = string + row.map(cell => `${cell.player ? cell.player : cell.visited ? 'X' : '_'}`)
    ...
    chooseMove(depthLimit, isMax, nextAction) {        
        ...
        for (let i = 0; i < possibleMoves.length; ++i) {
            let currMove = possibleMoves[i];
            let moveMax = this.chooseMove(!isMax ? depthLimit - 1 : depthLimit, !isMax, currMove);
